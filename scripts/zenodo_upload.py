...
METADATA = {
    ...
    "description": (
        "<p><b>Detecting alfalfa cutting timing from satellite to verify late-season "
        "water-saving opportunities in California</b> &mdash; analysis software and "
        "reproduction data for a remote-sensing study of alfalfa across the California "
        "Central Valley (10 counties, water years 2019&ndash;2024).</p>"
        "<p>The workflow detects alfalfa cutting (harvest) events from Harmonized "
        "Landsat&ndash;Sentinel (HLS) Enhanced Vegetation Index (EVI) time series using "
        "BEAST (Bayesian Estimator of Abrupt change, Seasonality and Trend; the Rbeast "
        "package), corrects OpenET actual evapotranspiration for post-cutting (off-phase) "
        "periods, and simulates the irrigation water savings achievable by delaying the "
        "last summer cutting past a 1 July cutoff. An independent PlanetScope cross-sensor "
        "validation of the detected cut dates is included. Development platform: "
        ...
        "<p><b>Licensing:</b> the source code (the .zip and the GitHub repository) is "
        "released under the <b>MIT license</b>; the data archives "
        "(intermediate_data_v1.tar.gz, analysis_full_v1.tar.gz) are released under "
        "<b>CC-BY 4.0</b>.</p>"
        "<p>Unpack the tarballs with <code>tar -xzf &lt;file&gt; -C &lt;repo root&gt;</code>. "
        "Raw satellite inputs (HLS EVI, Daymet, OpenET) are publicly re-downloadable from "
        "their original providers and are not included.</p>"
    ),
    "access_right": "open",
    "license": "cc-by-4.0",
    "version": "1.1.0",
