"""
BEAST number of seasonal change points boxplot
Horizontal boxplot showing distribution of seasonal change points per parcel across years.
"""
...
def plot_beast_n_cp_season_boxplot(
    county: str,
    years: Optional[List[int]] = None,
    output_dir: Optional[Path] = None,
    title: Optional[str] = None,
    x_label: str = "Number of Seasonal Change Points",
    figsize: tuple = (10, 6),
    dpi: int = 150
) -> dict:
    """
    Draw per-year horizontal boxplots for n_cp_season (or n_change_points if present instead).
    ...
    data, valid_years = provider.gather_year_values(county_norm, years, column="n_cp_season")
    
    if not data:
        return {
            "status": "error",
            "message": f"No CP-season data for {county_norm} in {years}",
            "file_path": None
        }
    
    labels = [str(y) for y in valid_years]
    out_path = output_dir / "boxplot_n_cp_season.png"
    
    if title is None:
        title = f"{county_norm}: Yearly Seasonal CP Distribution (grey=min..max; \u2022=mean)"
