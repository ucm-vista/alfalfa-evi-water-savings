"""
BEAST EVI with change points - observations scatter plot
Scatter plot of raw EVI observation points.
"""
...
def plot_beast_evi_with_cps_observations(
    county: str,
    wy: int,
    parcel: str,
    ...
    """
    Plot raw EVI observation points as scatter with change points marked as vertical lines.
    ...
    if mask.sum() == 0:
        return {
            "status": "error",
            "message": f"No valid observations for parcel {parcel}",
            "file_path": None
        }
    
    ax.scatter(parcel_data.loc[mask, "date"], parcel_data.loc[mask, "original_mean_evi"], 
              s=marker_size, alpha=alpha, label="Raw observations")
    ...
    ax.set_xlabel("Date")
    ax.set_ylabel("EVI")
    ax.set_title(f"{county} | Parcel {parcel} | WY {wy} (Raw Obs with CPs)")
    ax.legend()
    ax.grid(True, alpha=0.2)
    
    outfile = output_dir / f"WY{wy}_parcel_{parcel}_evi_observations_cps.png"
    plt.tight_layout()
