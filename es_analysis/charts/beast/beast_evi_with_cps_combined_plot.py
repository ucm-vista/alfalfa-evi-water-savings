"""
BEAST EVI with change points - combined plot
Multi-line plot showing original, gap-filled, and smoothed EVI with change points as vertical dashed lines.
"""
...
def plot_beast_evi_with_cps_combined(
    county: str,
    wy: int,
    parcel: str,
    ...
    """
    Plot all EVI versions (original, gap-filled, smoothed) with change point markers.
    ...
    Args:
        ...
        linewidths: Dict of linewidths for each line type (optional)
        alphas: Dict of alphas for each line type (optional)
    ...
    if linewidths is None:
        linewidths = {"original": 1.0, "gapfilled": 1.2, "smoothed": 1.5}
    if alphas is None:
        alphas = {"original": 1.0, "gapfilled": 0.9, "smoothed": 0.95}
    ...
    if "original_mean_evi" in parcel_data.columns:
        ax.plot(parcel_data["date"], parcel_data["original_mean_evi"], 
                label="Original (raw)", 
                linewidth=linewidths.get("original", 1.0))
    ...
    if "gapfilled_mean_evi" in parcel_data.columns:
        ax.plot(parcel_data["date"], parcel_data["gapfilled_mean_evi"], 
                label="Gap-filled (quartic)", 
                linewidth=linewidths.get("gapfilled", 1.2), 
                alpha=alphas.get("gapfilled", 0.9))
    
    if "smoothed_mean_evi" in parcel_data.columns:
        ax.plot(parcel_data["date"], parcel_data["smoothed_mean_evi"], 
                label="Smoothed (SG)", 
                linewidth=linewidths.get("smoothed", 1.5), 
                alpha=alphas.get("smoothed", 0.95))
    ...
    ax.set_xlabel("Date")
    ax.set_ylabel("EVI")
    ax.set_title(f"{county} | Parcel {parcel} | WY {wy} (EVI + CPs)")
    ax.legend()
    ax.grid(True, alpha=0.2)
    
    outfile = output_dir / f"WY{wy}_parcel_{parcel}_EVI_CP_combined.png"
    plt.tight_layout()
    ...
    return {
        ...
        "columns_available": [c for c in ["original_mean_evi", "gapfilled_mean_evi", "smoothed_mean_evi"] if c in parcel_data.columns]
    }