"""
EVI County Year Original Plot - Original EVI from loaded county/year CSV
Source reference: alfalfa_evi_jovyan.py line 743
"""
...
def plot_evi_county_year_original(county: str, wy: int, parcel_id: Optional[str] = None,
                                    output_dir: Optional[Path] = None) -> Path:
    """
    Plot original (raw) EVI from loaded county/year CSV
    
    Source: alfalfa_evi_jovyan.py line 743
    """
    ...
    for pid in to_plot:
        ...
        ax.set_xlabel("Date")
        ax.set_ylabel("EVI")
        ax.set_title(f"{county} | parcel {pid} | WY {wy} | win={INTERP_WINDOW_DAYS}d, SG={SG_WINDOW}/{SG_POLY}")
        ax.legend()
        ax.grid(True, alpha=0.2)

        outfile = out_dir / f"{county}_WY{wy}_parcel_{pid}_original.png"
        fig.tight_layout()
