"""
EVI Combined Plot - Combined plot showing original, gap-filled, and smoothed EVI
...
def plot_evi_combined(county: str, parcel_id: str, years: List[int],
                      output_dir: Optional[Path] = None,
                      interp_window: int = INTERP_WINDOW_DAYS,
                      sg_window: int = SG_WINDOW,
                      sg_poly: int = SG_POLY) -> Path:
    """
    Combined plot showing original, gap-filled, and smoothed EVI
    ...
    ax.plot(daily["date"], filled, label="Gap-filled (quartic)", linewidth=1.4, alpha=0.9)
    ax.plot(daily["date"], smoothed, label="Smoothed (SG)", linewidth=1.6, alpha=0.95)

    start, end = water_year_bounds_multi(years)
    ax.set_xlim(start, end)
    ax.set_xlabel("Date")
    ax.set_ylabel("EVI")
    title = f"{county} | parcel {parcel_id} | WY {min(years)}–{max(years)} | combined"
    ax.set_title(title)
    ax.legend()
    ...
    outfile = output_dir / (title.replace(" ", "_").replace("/", "_").replace("|", "_") + "_combined.png")
    fig.tight_layout()
