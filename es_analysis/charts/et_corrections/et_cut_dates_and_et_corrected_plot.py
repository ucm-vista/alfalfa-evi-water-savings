"""
ET Cut Dates and ET Corrected Plot

Monthly corrected ET side-by-side for water years.
"""
...
def et_cut_dates_and_et_corrected_plot(
    county: str,
    water_year: int,
    uid: str,
    method: str = "A",
    ...
    uid_index = {uid_i: i for i, uid_i in enumerate(uids)}
    for uid_i in uids:
        for cut_date in cuts_by_uid[uid_i]:
            crosses.append((cut_date, uid_i))
            same_day_counts[cut_date] = same_day_counts.get(cut_date, 0) + 1

            month_idx = monthly_df.index[monthly_df.index <= cut_date]
            if len(month_idx) > 0:
                last_month = month_idx[-1]
                et_corr = float(monthly_df.loc[last_month, "ET_corr"])
                dodge = (uid_index[uid_i] - (len(uids)-1)/2.0) * bar_dodge_days
                bars.append((last_month, et_corr, uid_i, dodge))

    fig, ax = plt.subplots(figsize=(fig_width, fig_height))
    bar_vals = [et for _, et, _, _ in bars if not (np.isnan(et) or np.isinf(et))]
    ymax = max(bar_vals + [1.0]) * 1.15

    for x, et_sum, uid_i, _ in bars:
        color = bar_color
        ax.bar(x, et_sum, width=pd.Timedelta(days=bar_width_days), color=color, alpha=bar_alpha, align="center", edgecolor="black", linewidth=0.8)
    ...
    ax.set_ylabel("ET Corrected between cuts (mm)")

    title_bits = [county_norm, f"WY {water_year}: cut dates (×) & Corrected ET (method {str(method).upper()})"]
    if filter_n_cp_season is not None:
        title_bits.append(f"| n_cp_season={filter_n_cp_season}")
    ax.set_title("  ".join(title_bits))

    handles = [mpl.lines.Line2D([0],[0], color=bar_color, lw=6, label=f"Corrected ET (method {str(method).upper()})")]
    handles += [mpl.lines.Line2D([0],[0], color=uid_colors[uid_i], lw=2, marker="x", markersize=8, linestyle="None") for uid_i in uids]
    ax.legend(handles, [f"Corrected ET (method {str(method).upper()})"] + uids, frameon=False,
              bbox_to_anchor=(1.02, 0.50), loc="center left", borderaxespad=0.0)

    fig.subplots_adjust(right=0.78)

    filename = f"{county_norm}_WY{water_year}_UID{uid}_cuts_x_et_corrected_method{method.upper()}.png"
    if filter_n_cp_season is not None:
        filename = f"{county_norm}_WY{water_year}_UID{uid}_ncp{filter_n_cp_season}_cuts_x_et_corrected_method{method.upper()}.png"
    ...
    return {
        ...
        "figure_type": "cut_dates_and_et_corrected",
    }


if __name__ == "__main__":
    result = et_cut_dates_and_et_corrected_plot("Fresno", 2020, "1003334", "A", filter_n_cp_season=10)
