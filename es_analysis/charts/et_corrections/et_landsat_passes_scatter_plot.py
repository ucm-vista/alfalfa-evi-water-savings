"""
ET Landsat Passes Scatter Plot

Displays Landsat observation passes with diamond markers.
"""
...
def et_landsat_passes_scatter_plot(
    county: str,
    water_year: int,
    uid: str,
    method: str = "A",
    ...
    ax.set_title(f"{county_norm} | WY{water_year} | UID {uid} – Landsat Passes (≤ {cloud_cover_max:.0f}% CC)")
    ax.legend()

    output_path = output_dir / "et_corrections" / f"{county_norm}_WY{water_year}_UID{uid}_landsat_passes.png"
    ...
    return {
        ...
        "figure_type": "landsat_passes_scatter",
    }


if __name__ == "__main__":
    result = et_landsat_passes_scatter_plot("Fresno", 2020, "1003334", "A", 20.0)
