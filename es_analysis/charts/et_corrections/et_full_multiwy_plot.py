"""
ET Full Multi-WY Plot

Multi-panel multi-year plot spanning multiple water years.
"""
...
def et_full_multiwy_plot(
    ...
    start = water_year_bounds(int(wy_start))[0]
    end = water_year_bounds(int(wy_end))[1]
    ...
    for wy in range(int(wy_start), int(wy_end) + 1):
        try:
            ...
            harvest_all.extend(harvest_dates)
            passes_list.append(passes_df)
        except Exception:
            continue

    if not daily_list:
        raise ValueError(f"No valid data for UID {uid} in WY{wy_start}-WY{wy_end}")

    daily_df = pd.concat(daily_list).sort_index()
    monthly_df = pd.concat(monthly_list).sort_index()
    passes_df = pd.concat(passes_list).drop_duplicates(subset=["date_only"]).reset_index(drop=True) if passes_list else pd.DataFrame()
    ...
    if harvest_all:
        ax.scatter(
            harvest_all,
            [y_max_month * 0.93] * len(harvest_all),
            marker="x",
            s=90,
            color="black",
            linewidths=2.0,
    ...
    n_months = max(int(len(monthly_df.index)), 1)
    interval = 1 if n_months <= 18 else (3 if n_months <= 72 else 6)
    ...
    ax.set_title(
        f"{county_norm} | WY{wy_start}–WY{wy_end} | UID {uid} – Off-phase correction (method {str(method).upper()})"
    )
    ...
    output_path = output_dir / "et_corrections" / f"{county_norm}_WY{wy_start}_WY{wy_end}_UID{uid}_multiwy.png"
    ...
    return {
        ...
        "method": method.upper(),
        "ci_alpha": ci_alpha,
        "n_harvest_dates": len(harvest_all),
...
if __name__ == "__main__":
    result = et_full_multiwy_plot("Fresno", 2019, 2021, "1003334", "A", 0.10, True, -0.10, 20.0)
