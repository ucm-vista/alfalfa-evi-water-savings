"""
ET Monthly OpenET Bar Plot

Generates a bar chart showing monthly OpenET totals.
"""
...
def et_monthly_openet_bar_plot(
    county: str,
    water_year: int,
    uid: str,
    method: str = "A",
    ...
    y_max = float(np.nanmax(monthly_df["ET_open"].to_numpy())) * 1.25
    ...
    ax.set_title(f"{county_norm} | WY{water_year} | UID {uid} – Monthly OpenET")
    ax.legend()

    output_path = output_dir / "et_corrections" / f"{county_norm}_WY{water_year}_UID{uid}_monthly_openet.png"
    ...
    return {
        ...
        "uid": uid,
        "method": method.upper(),
        "figure_type": "monthly_openet_bar",
    }


if __name__ == "__main__":
    result = et_monthly_openet_bar_plot("Fresno", 2020, "1003334", "A")
